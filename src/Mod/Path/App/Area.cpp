...
struct WireJoiner {
    ...
    void splitEdges(double tolerance = Precision::Confusion()) {
        ...
        for(auto it=edges.begin();it!=edges.end();) {
                ...
                ++vit)
            {
                ...
                for(int i=0; i<2; ++i) {
                    ...
                    if(pstart.SquareDistance(p)<=squareTolerance ||
                       pend.SquareDistance(p)<=squareTolerance)
                        continue;
                    ...
                    if(extss.IsDone() && extss.NbSolution()) {
                        ...
                        if(pp.SquareDistance(p)<=squareTolerance) {
                            pt = pp;
                            intersects = true;
                            break;
                        }
...
#define ADD_HLR_SHAPE(_name) \
    ...
    // joiner.splitEdges(Precision::Confusion());
